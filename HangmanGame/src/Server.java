...
public class Server extends javax.swing.JFrame {
    ...
    public static void main(String args[]) throws FileNotFoundException, IOException, UnsupportedAudioFileException, LineUnavailableException {
        ...
        // Create a server socket (TCP socket used)
        ...
        try {
            while (true) {
                ...
                // Accept connection of player 1 (TCP socket used)
                Socket player1Socket = serverSocket.accept();
                new DataOutputStream(player1Socket.getOutputStream()).writeInt(1); //inform the player he/she is number one
                textBox.append("\nPlayer 1 has joined the game!\n");
                textBox.append("\nWaiting for player 2 to join the game...\n");

                // Accept connection of player 2 ((TCP socket used))
                Socket player2Socket = serverSocket.accept();
                new DataOutputStream(player2Socket.getOutputStream()).writeInt(2); //inform the player he/she is number two
                ...
                // starting a new thread for HangmanGame to start a game with only 2 players
...
class HangmanGame extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            // Start a data input/output streams to receive/send information from/to players
            ...
            do {
                ...
                // The players' attempts are the length of the word + 2 more tries
            ...
            if (player1Attempts > 0) {
                ...
                if (checkCorrect(answer)) {
                    if (answer.length() == 1 && exist) {
                        toPlayer1.writeUTF("The letter '" + answer + "' is already answered before!");
    ...
    // This method chooses a random word
    private void randomWord() throws IOException {

        // Choose random line (we have 107 words to begin with, will be decreased by 1 every time we pick a word)
    ...
    private boolean checkCorrect(String answer) {
        ...
        if (answer.equalsIgnoreCase(randomWord)) {
            return true;
        } else if (answer.length() == 1 && randomWord.contains(answer)) { // If player entered a char not a word
    ...
    private boolean alreadyExist(char answer) {
        // This method will check if the letter was already revealed
        for (int i = 0; i < encryptedWord.length; i++) {
